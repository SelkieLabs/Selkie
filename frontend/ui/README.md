...
- `src/styles/theme.css`: design tokens (color, radius, spacing) and the base component styles.
- `src/components/`: reusable primitives (`Button`, `Card`, `Money`).

## Migration plan

The web app (`frontend/web`) still carries its own components and styles from the Canton build. As we build on Stellar, we promote the genuinely reusable ones (the money display, transaction rows, the reveal and scene motion, layout) into this package and have the web app import them from `@selkie/ui`. New shared pieces start here from day one.
