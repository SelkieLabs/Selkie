    ...
    <section className="chunk-gold p-6 sm:p-7">
      ...
      <div className="mt-7 flex flex-wrap items-end justify-between gap-5">
        ...
        <div className="flex gap-2.5">
          <Link href="/dashboard/send" className="btn btn-dim btn-sm">
...
function Dashboard() {
  ...
  if (!tab || !TABS.some((t) => t.id === tab)) return <Redirect to="/dashboard/activity" />;
            ...
            >
              {TABS.map((t) => (
                <Link
                  ...
                  className={`rail-sq ${t.id === tab ? "rail-on" : ""}`}
                >
                  {t.icon}
...
/**
 * `?to=` on the send tab is read with `useSearchParams`, which needs a Suspense
 * boundary to stay out of the way of the route's prerendered shell.
 */
